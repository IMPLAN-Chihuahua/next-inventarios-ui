...
export default function ResguardoExterno({ controlledOpen, onControlledClose, hideLauncher = false }: Props) {
  ...
  const steps: DrawerStep[] = [
    { label: "Datos del préstamo", validate: validateGeneral, content: () => <>{error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}<Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 1.5, p: 2, bgcolor: "#fff", border: "1px solid #e4eaec", borderRadius: "16px" }}><DrawerSectionTitle icon={<Handshake size={18} />} title="Datos del préstamo" description="Identifica el documento y a quien recibe los bienes." /><TextField required label="Folio" value={folio} onChange={(e) => setFolio(sanitizeSafeText(e.target.value))} sx={drawerFieldStyles} /><TextField required label="Fecha" type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} sx={drawerFieldStyles} slotProps={{ inputLabel: { shrink: true } }} /><TextField required label="Persona o empresa que recibe" value={receptor} onChange={(e) => setReceptor(sanitizeSafeText(e.target.value))} sx={{ ...drawerFieldStyles, gridColumn: { sm: "1/-1" } }} /><TextField label="Comentarios opcionales" value={comentarios} onChange={(e) => setComentarios(sanitizeSafeText(e.target.value))} multiline minRows={3} sx={{ ...drawerFieldStyles, gridColumn: { sm: "1/-1" } }} /></Box></> },
    { label: "Seleccionar bienes", validate: validateArticles, content: () => <>{error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}<Box sx={{ p: 2, bgcolor: "#fff", border: "1px solid #e4eaec", borderRadius: "16px" }}><DrawerSectionTitle icon={<PackageSearch size={18} />} title="Bienes en préstamo" description="Busca y agrega los artículos que saldrán de la institución." /><Autocomplete options={options} loading={searching} filterOptions={(items) => items} inputValue={query} value={null} getOptionLabel={(o) => o.numeroInventario + " · " + (o.descripcion || "Sin descripción")} isOptionEqualToValue={(o,v) => o._id === v._id} onInputChange={(_,v) => setQuery(v)} onChange={(_,v) => { if (!v) return; setArticulos((current) => [...current,v]); setQuery(""); setOptions([]); setError(""); }} noOptionsText={query.trim().length < 2 ? null : "Sin coincidencias"} renderInput={(params) => <TextField {...params} placeholder="Buscar por inventario, serie o descripción" sx={{ ...drawerFieldStyles, mt: 1.5 }} />} />{articleList}</Box></> },
